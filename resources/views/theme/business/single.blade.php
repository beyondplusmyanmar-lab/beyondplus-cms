        ...
        <article class="col-lg-8">
            <div class="d-flex justify-content-between align-items-start">
                ...
                @if(Auth::guard('admins')->check() && $post->post_type == 'post')
                    <a href="{{ url('/bp-admin/post/'.$post->id.'/edit') }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i> Edit</a>
                @endif
            </div>
            ...
            @if($postCategories->count())
                <div class="mb-3">
                    @foreach($postCategories as $cat)
                        <a href="{{ url('/cat/'.$cat->tax_link) }}" class="badge text-white text-decoration-none me-1" style="background:var(--bz-primary);">{{ $cat->tax_name }}</a>
                    @endforeach
                </div>
            @endif
            ...
            @auth
                <hr class="my-4">
                <h5 class="mb-3">{{ $mm ? 'မှတ်ချက်များ' : 'Comments' }}</h5>
                <div class="d-flex gap-2 mb-4">
                    <img src="{{ Auth::user()->avatar ?: asset('/img/blank_profile_pic_60x60.jpg') }}" class="rounded-circle" width="48" height="48" alt="you">
                    ...
                    @foreach($post->comment as $c)
                        ...
                        @if($author)
                            <div class="d-flex gap-2 mb-3">
                                <img src="{{ $author->avatar ?: asset('/img/blank_profile_pic_60x60.jpg') }}" class="rounded-circle" width="40" height="40" alt="{{ $author->name }}">
                                <div>
                                    <strong>{{ $author->name }}</strong>
                                    <span class="bz-muted small">&middot; {{ $c->created_at->diffForHumans() }}</span>
                                    <div>{{ $c->body }}</div>
                                </div>
                            </div>
                        @endif
