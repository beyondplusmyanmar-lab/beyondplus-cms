...
<style>
    .bz-cal { width:100%; border-collapse:collapse; table-layout:fixed; }
    .bz-cal th { padding:.5rem; text-align:center; font-size:.72rem; text-transform:uppercase; letter-spacing:.4px; color:var(--bz-muted); border-bottom:1px solid var(--bz-border); }
    .bz-cal td { border:1px solid var(--bz-border); vertical-align:top; height:112px; padding:.35rem; overflow:hidden; }
    .bz-cal .daynum { font-size:.8rem; color:var(--bz-muted); font-weight:600; }
    .bz-cal td.out { background:var(--bz-surface); } .bz-cal td.out .daynum { color:#cbd5e1; }
    .bz-cal td.today .daynum { display:inline-flex; align-items:center; justify-content:center; min-width:1.4rem; height:1.4rem; background:var(--bz-primary); color:#fff; border-radius:50%; }
    .bz-cal .ev { display:block; background:var(--bz-primary); color:#fff; font-size:.72rem; line-height:1.3; padding:2px 6px; border-radius:4px; margin-top:3px; text-decoration:none; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .bz-cal .ev:hover { background:var(--bz-primary-dark); color:#fff; }
    .bz-cal .ev .t { opacity:.85; font-weight:600; margin-right:3px; }
    @media (max-width:640px){ .bz-cal td { height:76px; } .bz-cal .ev { font-size:.6rem; } }
</style>
<div class="container py-5">
    <div class="d-flex align-items-center justify-content-between flex-wrap mb-4" style="gap:.5rem;">
        <h1 class="h3 mb-0">{{ $mm ? 'ပွဲများ' : 'Events' }}</h1>
