...
@section('content')
    ...
    @if (empty($lines))
        <div class="card muted" style="padding:20px;">
            ...
            <a href="{{ url('/store') }}">{{ $mm ? 'ဈေးဆိုင်သို့ →' : 'Back to the shop →' }}</a>
        </div>
    @else
        ...
        <form method="POST" action="{{ url('/store/checkout') }}" class="card" style="padding:20px;">
            @csrf
        ...
        <p style="margin-top:14px;"><a href="{{ url('/store') }}">{{ $mm ? '← ဆက်ကြည့်ရန်' : '← Keep shopping' }}</a></p>
    @endif
@endsection
