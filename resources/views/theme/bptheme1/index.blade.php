                    ...
                    <article class="md-story h-100">
                        @if($p->featured_img)
                            <a href="{{ url('/'.$p->post_link) }}">
                                <img src="{{ bp_upload_url($p->featured_img) }}" class="md-story-img rounded-1 mb-3" alt="{{ $p->title }}">
                            </a>
                        @endif
                        @if($c->tax_name)
                ...
                @foreach($sliders->take(3) as $slide)
                    <div class="col-md-4">
                        <a href="{{ $slide->slider_url ?: '#' }}" class="d-block position-relative">
                            <img src="{{ bp_upload_url($slide->slider_link) }}" class="w-100 rounded-1" style="aspect-ratio:16/9;object-fit:cover;" alt="{{ $slide->slider_name }}">
