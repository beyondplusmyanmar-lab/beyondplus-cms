...
function module($sectionId)
{
    ...
    if ($user_type == 0) {
        $query = $query->where('group_access->user', '1');
    }
...
function languageId()
{
    ...
    $id = 0;
    $languages = Bp_languages::where('language_iso', App::getLocale())->first();
...
function english_digits_to_myanmar_words($english_digits)
{
    ...
    $myanmar_counting_number_array = ['သုည', 'တစ်', 'နှစ်', 'သုံး', 'လေး', 'ငါး', 'ခြောက်', 'ခုနစ်', 'ရှစ်', 'ကိုး'];
    $myanmar_counting_number_suffixes_array = ['ဆယ်', 'ရာ', 'ထောင်', 'သောင်း', 'သိန်း'];
...
{
    return bp_option(Plugin::settingKey($slug, $name), $default);
}

function bp_add_action($hook, $cb, $priority = 10)
{
    Plugin::addAction($hook, $cb, $priority);
}
...
{
    Plugin::addFilter($hook, $cb, $priority);
}
...
function role_type($type = null)
{
    ...
    $role_type = Bp_usertype::get()->pluck('role', 'id');
...
function bbParse($string)
{
    ...
    while (preg_match_all('`\[('.$tags.')=?(.*?)\](.+?)\[/\1\]`', $string, $matches)) {
        foreach ($matches[0] as $key => $match) {
            [$tag, $param, $innertext] = [$matches[1][$key], $matches[2][$key], $matches[3][$key]];
            switch ($tag) {
                case 'block': $replacement = showBlock($innertext);
                    break;
            }
