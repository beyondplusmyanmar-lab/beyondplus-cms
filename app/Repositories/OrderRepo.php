...
use App\Mail\Outofstock_adminMail;
...
class OrderRepo
{
    ...
    public function stockControl($order, $status)
    {
        ...
        if (count($outofstock) > 0) {
            ...
            Mail::to('admin@example.com')->send(new Outofstock_adminMail($product));
        }
    ...
    public function order_processing_mail($id)
    {
        ...
        if ($response) {
            ...
            Mail::to($email)->send(new OrderSendMail($order, $order_mail_status, 1));
            Mail::to('admin@example.com')->send(new OrderSendMail($order, $order_mail_status, 2));
